...
function InitProducts(csrf, urls) {
  ...
  document.getElementById('mprod-category').addEventListener('change', () => {
    const cat = document.getElementById('mprod-category').value;
    document.getElementById('mprod-attr-category').value = cat;
    loadAttrsForModal(cat);
  });
}
...
function renderTable() {
  ...
  tbody.innerHTML = slice.map(p => {
    const attrCount = p.attributes?.length ?? p.attribute_count ?? '—';
    return `<tr>
      <td>
