...
class ListingViewSet(viewsets.ViewSet):
    ...
    @check_refurbisher_profile()
    def list(self, request):
        """List listings - refurbishers see only their own, admins see all with filters"""
        status_filter = request.query_params.get('status')
        search = request.query_params.get('search')
        
        # Check if user is admin
        is_admin = request.user.is_staff or getattr(request.user, 'user_role', None) == 'admin'
        
        if is_admin:
            queryset = Listing.objects.all()
        else:
            queryset = Listing.objects.filter(refurbisher=request.user)
        
        queryset = queryset.select_related(
            'model', 'model__brand', 'refurbisher', 'approved_by'
        ...
        if search:
            queryset = queryset.filter(
                ...
                Q(refurbisher__name__icontains=search) |
    ...
    def retrieve(self, request, pk=None):
        ...
        listing = get_object_or_404(
            Listing.objects.select_related('model', 'model__brand', 'refurbisher', 'approved_by')
    ...
    def update(self, request, pk=None):
        ...
        # Allow updating status only if draft
        if listing.status != 'draft':
            return Response({
                "success": False, "user_not_logged_in": False, "user_unauthorized": False,
                "data": None, "error": "Can only update draft listings."
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def submit_for_approval(self, request, pk=None):
        """Refurbisher submits listing for admin approval"""
        listing = get_object_or_404(Listing, pk=pk, refurbisher=request.user)
        
        if listing.status != 'draft':
            return Response({
                "success": False, "user_not_logged_in": False, "user_unauthorized": False,
                "data": None, "error": "Only draft listings can be submitted for approval."
            }, status=status.HTTP_400_BAD_REQUEST)
